...
class Load_calculation:
    ...
    def calculate_poly_loads(self, kd_factor): #stringer crippling stress is the average 
        ...
        """
        # Step 1: Find valid thickness indices that satisfy the geometric constraint for *all* elements
        valid_thickness_indices = []

        for n in range(len(self.geometry.t)):
            valid = True
            for i in range(len(self.geometry.elements)):
                if self.geometry.stringers[i] > 0:  # only check if there are stringers on this edge
                    if self.geometry.elements[i] < 2 * self.w_e[n] * self.geometry.stringers[i]:
                        valid = False
                        break  # no need to check further elements for this thickness
            if valid:
                valid_thickness_indices.append(n)

        if len(valid_thickness_indices) == 0:
            raise ValueError("No valid thickness found that fits stringers on all edges")


    
        self.geometry.t = self.geometry.t[valid_thickness_indices]
        print(self.geometry.t[valid_thickness_indices], self.geometry.t)
        self.w_e = self.w_e[valid_thickness_indices]
...
if __name__ == "__main__":
    ...
    safety_mass_margin = 1.5 #for joints, discontinuities etc.
    ...
    best_config = None
    w_mass = 0.5          # mass is critical for launch
