...
class PropulsionProperties:
    ...
    BIPROP = {'name': 'Bipropellant Propulsion',
              ...
              'fuel_tank_mass': 15,  #kg
              'oxidiser_tank_mass': 15,  #kg
              'pressurant_tank_mass': 14,  #kg

              'fuel_tank_volume': 0.138,  # m^3
              'oxidiser_tank_volume': 0.138,  # m^3
              'pressurant_tank_volume': 0.108,  # m^3
...
biprop_base_mass = (
    ...
    2 * biprop['fuel_tank_mass'] + 2 * biprop['oxidiser_tank_mass'] +biprop['pressurant_tank_mass']
)
...
print(f"Mass before station-keeping: {m_before_stkeeping:.1f} kg")
...
# Check if biprop propellants fit in available tank volumes
max_fuel_capacity = 2*biprop['fuel_tank_volume'] * biprop['fuel_density']
max_oxidizer_capacity = 2*biprop['oxidiser_tank_volume'] * biprop['oxidizer_density']
max_pressurant_capacity = 2*biprop['pressurant_tank_volume'] * biprop['press_density']

print(f"\nBiprop tank capacity check:")
print(f"Fuel capacity: {max_fuel_capacity:.1f} kg (total fuel tanks)")
print(f"Fuel required: {fuel_mass:.1f} kg")
if fuel_mass > max_fuel_capacity:
    print(f"WARNING: Required fuel ({fuel_mass:.1f} kg) exceeds tank capacity!")
else:
    print(f"Fuel fits in tanks with {max_fuel_capacity - fuel_mass:.1f} kg margin")

print(f"\nOxidizer capacity: {max_oxidizer_capacity:.1f} kg (total oxidizer tanks)")
print(f"Oxidizer required: {oxidizer_mass:.1f} kg")
if oxidizer_mass > max_oxidizer_capacity:
    print(f"WARNING: Required oxidizer ({oxidizer_mass:.1f} kg) exceeds tank capacity!")
else:
    print(f"Oxidizer fits in tanks with {max_oxidizer_capacity - oxidizer_mass:.1f} kg margin")

print(f"\nPressurant capacity: {max_pressurant_capacity:.1f} kg")
print(f"Pressurant required: {press_mass:.1f} kg")
if press_mass > max_pressurant_capacity:
    print(f"WARNING: Required pressurant ({press_mass:.1f} kg) exceeds tank capacity!")
else:
    print(f"Pressurant fits in tank with {max_pressurant_capacity - press_mass:.1f} kg margin")
...
# Final verification
propellants_only = m_prop_stkeeping_electric + m_prop_capture + press_mass
